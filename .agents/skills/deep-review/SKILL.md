---
name: deep-review
description: Deep, production-critical review workflow for Kallisto changes. Use when asked to review a PR, branch, commit range, or diff in this repository and produce a markdown review report with findings, risk analysis, and Kallisto-specific validation results.
---
...
Produce a production-critical review for `Naughtian Kallisto` that:
...
- follows repository rules from `AGENTS.md`
...
- Treat `make format` and `make clippy` as the authoritative static checks because Kallisto’s `Makefile` adds required setup and repository-specific scripts.
...
4. Read the affected `Kallisto` subsystems
...
7. Run `Kallisto` static validation from `./Makefile`
