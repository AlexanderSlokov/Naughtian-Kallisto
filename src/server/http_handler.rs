...
use crate::engine::{engine_registry::EngineRegistry, error::EngineError, traits::SecretPayload};
...
fn parse_versions_list(body: &[u8]) -> Vec<u32> {
    ...
    if let Ok(root) = sonic_rs::from_slice::<Value>(body)
        && let Some(arr) = root
            .pointer(sonic_rs::pointer!["versions"])
...
fn extract_version_param(uri: &Uri) -> u32 {
    uri.query()
        .and_then(|q| {
            q.find("version=").map(|i| &q[i + 8..]).and_then(|s| {
...
fn extract_depth_param(uri: &Uri) -> u32 {
    uri.query()
        .and_then(|q| {
            q.find("depth=").map(|i| &q[i + 6..]).and_then(|s| {
                let end = s.find('&').unwrap_or(s.len());
                s[..end].parse::<u32>().ok()
            })
        })
...
async fn read_secret(
    ...
    let (mount, path) =
        extract_mount_and_path(uri.path(), "data").ok_or(AppError::MountNotFound)?;
    let engine = state
        .registry
        .resolve(mount)
    ...
    let deletion_time = if meta.deletion_time_ms > 0 {
        format!(
            "\"{}\"",
            crate::server::time_format::epoch_ms_to_rfc3339(meta.deletion_time_ms)
        )
    } else {
        "\"\"".to_string()
    };
...
fn json_merge_patch(target: &mut sonic_rs::Value, patch: &sonic_rs::Value) {
    ...
    for (key, value) in patch_obj.iter() {
        if value.is_null() {
            ...
            // Note: entry() and or_insert() are not available on sonic_rs::Object directly
            // like this, we have to check if it exists.
...
async fn patch_secret(
    ...
    let mut current_value = sonic_rs::from_str::<sonic_rs::Value>(&current_payload.value)
        .unwrap_or_else(|_| sonic_rs::json!({}));

    let patch_body = sonic_rs::from_slice::<sonic_rs::Value>(body.as_ref()).map_err(|_| {
        AppError::Engine(EngineError::StorageError("Invalid patch JSON".to_string()))
    })?;

    let patch_data = patch_body
        .pointer(sonic_rs::pointer!["data"])
...
async fn read_subkeys(
    ...
    let (mount, path) =
        extract_mount_and_path(uri.path(), "subkeys").ok_or(AppError::MountNotFound)?;
    let engine = state
        .registry
        .resolve(mount)
        .ok_or(AppError::MountNotFound)?;
...
async fn write_secret(
    ...
    // Sử dụng Lazy Evaluation của sonic-rs để trích xuất Zero-Copy chuỗi "data"
    // (Safe Rust)
...
async fn soft_delete_versions(
    ...
    let (mount, path) =
        extract_mount_and_path(uri.path(), "delete").ok_or(AppError::MountNotFound)?;
    let engine = state
        .registry
        .resolve(mount)
        .ok_or(AppError::MountNotFound)?;
...
async fn undelete_versions(
    ...
    let (mount, path) =
        extract_mount_and_path(uri.path(), "undelete").ok_or(AppError::MountNotFound)?;
    let engine = state
        .registry
        .resolve(mount)
        .ok_or(AppError::MountNotFound)?;
...
async fn destroy_versions(
    ...
    let (mount, path) =
        extract_mount_and_path(uri.path(), "destroy").ok_or(AppError::MountNotFound)?;
    let engine = state
        .registry
        .resolve(mount)
        .ok_or(AppError::MountNotFound)?;
...
async fn read_metadata(
    ...
    let (mount, path) =
        extract_mount_and_path(uri.path(), "metadata").ok_or(AppError::MountNotFound)?;
    let engine = state
        .registry
        .resolve(mount)
        .ok_or(AppError::MountNotFound)?;
    ...
    response.push_str(if metadata.cas_required {
        "true"
    } else {
        "false"
    });
    ...
    response.push_str(&crate::server::time_format::ms_to_vault_duration(
        metadata.delete_version_after_ms,
    ));
    ...
    for (i, v) in metadata.versions.iter().enumerate() {
        ...
        response.push_str(r#""created_time":""#);
        response.push_str(&crate::server::time_format::epoch_ms_to_rfc3339(
            v.created_time_ms,
        ));
        response.push_str(r#"","deletion_time":"#);
        ...
        let (status, err_msg) = match self {
            ...
            AppError::Engine(e) => match e {
                ...
                EngineError::SoftDeleted => {
                    (StatusCode::NOT_FOUND, "Secret soft-deleted".to_string())
                }
                EngineError::Destroyed => (StatusCode::NOT_FOUND, "Key destroyed".to_string()),
                EngineError::InvalidVersion(v) => {
                    (StatusCode::NOT_FOUND, format!("Invalid version: {}", v))
                }
...
mod tests {
    ...
    fn test_extract_mount_and_path() {
        ...
        let (mount, path) =
            extract_mount_and_path("/v1/kv2/metadata/deep/path/key", "metadata").unwrap();
        assert_eq!(mount, "kv2");
