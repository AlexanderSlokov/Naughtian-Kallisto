...
pub fn epoch_ms_to_rfc3339(ms: u64) -> String {
    ...
    // Using simple format! since we don't have itoa in deps, but pre-allocated buf minimizes impact.
    use std::fmt::Write;
    if fraction_ms > 0 {
        let _ = write!(&mut buf, "{:04}-{:02}-{:02}T{:02}:{:02}:{:02}.{:03}Z", year, month, d, h, m, s, fraction_ms);
    } else {
        let _ = write!(&mut buf, "{:04}-{:02}-{:02}T{:02}:{:02}:{:02}Z", year, month, d, h, m, s);
    }
...
mod tests {
    ...
    fn test_epoch_ms_to_rfc3339() {
        ...
        assert_eq!(epoch_ms_to_rfc3339(1672531200123), "2023-01-01T00:00:00.123Z");
    }

    #[test]
    fn test_parse_vault_duration() {
        assert_eq!(parse_vault_duration("3h").unwrap(), 3 * 3600 * 1000);
        assert_eq!(parse_vault_duration("3h25m19s").unwrap(), (3 * 3600 + 25 * 60 + 19) * 1000);
    ...
    fn test_ms_to_vault_duration() {
        ...
        assert_eq!(ms_to_vault_duration((3 * 3600 + 25 * 60 + 19) * 1000), "3h25m19s");
    }
}
