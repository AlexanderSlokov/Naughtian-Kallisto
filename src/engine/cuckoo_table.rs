        ...
        unsafe {
            ...
            for i in 0..8 {
                ...
                if slot.index != u32::MAX && slot.tag == tag {
                    ...
                    if se.key == key {
                        *se = SecretEntry { key: key.to_string(), payload: entry.payload };
                        return true;
                    }
                }
            }
            ...
            if state.free_list_size > 0 {
                ...
                self.shadow_free_list_size.store(state.free_list_size, Ordering::Relaxed);
                
                let se = &mut *state.storage.add(new_storage_idx as usize);
                *se = SecretEntry { key: key.to_string(), payload: entry.payload.clone() };
            } else {
                if state.storage_size == state.storage_capacity {
                    ...
                    let new_ptr = realloc(state.storage as *mut u8, old_layout, new_cap * std::mem::size_of::<SecretEntry>()) as *mut SecretEntry;
                    if new_ptr.is_null() {
                        panic!("OOM in CuckooTable storage");
                    }
                    state.storage = new_ptr;
                    state.storage_capacity = new_cap;
                    self.shadow_storage_capacity.store(new_cap, Ordering::Relaxed);
                }
                
                new_storage_idx = state.storage_size as u32;
                state.storage_size += 1;
                self.shadow_storage_size.store(state.storage_size, Ordering::Relaxed);
                
                ptr::write(state.storage.add(new_storage_idx as usize), SecretEntry { key: key.to_string(), payload: entry.payload.clone() });
            }
                ...
                if slot.index != u32::MAX && slot.tag == tag {
                    ...
                    if se.key == key {
                        ...
                        if state.free_list_size == state.free_list_capacity {
                            ...
                            let old_layout = Layout::array::<u32>(state.free_list_capacity).unwrap();
                            let new_ptr = realloc(state.free_list as *mut u8, old_layout, new_cap * std::mem::size_of::<u32>()) as *mut u32;
